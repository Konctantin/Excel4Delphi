unit Excel4Delphi.NumberFormats;
...
var
  retPos: integer;
begin
  Result := IsHaveSymbolsAfterPosQuotas(AStartPos, ALen, retPos, AStr, SymbolsArr);
end; // IsHaveSymbolsAfterPosQuotas
      ...
      case (ch) of
        '0', '#', 'E', 'e', '%', '?':
          begin
            ...
            else if (IsHaveSymbolsAfterPosQuotas(i - 1, l, FormatStr, ['%'])) then
              Result := Result or ZE_NUMFORMAT_NUM_IS_PERCENTAGE
            else if (IsHaveSymbolsAfterPosQuotas(i - 1, l, FormatStr, ['/']) or _isFraction) then
      ...
      case (ch) of
        '0', '#', '?':
          begin
            ...
            else if (IsHaveSymbolsAfterPosQuotas(i - 1, l, t, FormatStr, ['%'])) then
            begin
              Result := Result or ZE_NUMFORMAT_NUM_IS_PERCENTAGE;
              i := t;
            end
...
var
  ...
  begin
    ...
    if ((z = 'YY') or (z = 'YYYY')) then
      ...
      _AddToResult('ss', 'S')
    else if ((z = 'MMM') or (z = 'MMMM') or (z = 'MMMMM')) then
      _AddToResult(z, 'M')
    else
    // Minute or Month?
  ...
  begin
    ...
    if (z = 'COLOR1') then
      s := 'BLACK'
    else if (z = 'COLOR2') then
      s := 'WHITE'
    else if (z = 'COLOR3') then
      s := 'RED';
...
begin
  ...
  begin
    ...
    else if (((_IsQuote and (ch <> '"')) or (_IsBracket and (ch <> ']')))) then
      s := s + ch
    else
      case (ch) of
        '[':
        ...
        'y', 'Y':
          _ProcessDateTimeSymbol('Y');
        'm', 'M':
          _ProcessDateTimeSymbol('M');
        'd', 'D':
          _ProcessDateTimeSymbol('D');
        's', 'S':
          _ProcessDateTimeSymbol('S');
        'h', 'H':
          _ProcessDateTimeSymbol('H');
        '\':
          begin
            _CheckStringItem(ch);
            Result := Result + ch;
  ...
  begin
    ...
    case (ADateType) of
      ...
      ZE_DATETIME_ITEM_QUARTER:
        Result := false;
 *)
      ZE_DATETIME_ITEM_AMPM, ZE_DATETIME_ITEM_HOUR, ZE_DATETIME_ITEM_SECOND:
        Result := true;
    end;
  end; // _CheckNeighbors
...
var
  ...
  begin
    ...
    if (_tmp = 'G') then
      t := 1
    else if (_tmp = 'GG') then
      t := 2
    else if (_tmp = 'GGG') then
      t := 3
    else if (_tmp = 'GGGEE') then
      t := 4;
...
procedure TZEODSNumberFormatReader.AddEmbededText(const AText: string; ANumberPosition: integer);
var
  i, _pos: integer;
begin
...
begin
  ...
  while Xml.ReadToEndTagByName(ATagName) do
  begin
    // Day
    if ((Xml.TagName = ZETag_number_day) and (Xml.IsTagOfData)) then
...
begin
  ...
  else if (Xml.TagName = ZETag_number_currency_style) then
    ReadCurrencyFormat(Xml)
  else if (Xml.TagName = ZETag_number_percentage_style) then
    ReadPercentageFormat(Xml)
  else if (Xml.TagName = ZETag_number_date_style) then
    ReadDateFormat(Xml, ZETag_number_date_style)
  else if (Xml.TagName = ZETag_number_time_style) then
    ReadDateFormat(Xml, ZETag_number_time_style)
  else if (Xml.TagName = ZETag_number_text_style) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if ((Xml.IsTagEnd) and (_number_position >= 0)) then
 ...
 *)
  ...
  begin
    ...
    if ((_min_numerator_digits > 0) and (_min_denominator_digits > 0)) then
      _result := _result + ' ' + _GetRepeatedString(_min_numerator_digits, '?') + '/' +
  ...
  begin
    ...
    begin
      _result := _result + _GetRepeatedString(_min_int_digits, '0') + ZE_NUMBER_FORMAT_DECIMAL_SEPARATOR +
        _GetRepeatedString(_decimalPlaces, '0') + 'E+' + _GetRepeatedString(_min_exponent_digits, '0');
...
begin
  ...
  begin
    ...
    if (Xml.TagName = ZETag_number_number) then
      _ReadNumber_Number()
    else if (Xml.TagName = ZETag_number_fraction) then
      _ReadNumber_Fraction()
    else if (Xml.TagName = ZETag_number_scientific_number) then
      _ReadNumber_Scientific()
    else if (Xml.TagName = ZETag_number_currency_symbol) then
      _ReadCurrecny_Symbol()
    else if ((Xml.TagName = ZETag_number_text) and (Xml.IsTagEnd)) then
    begin
    ...
    else if (Xml.TagName = ZETag_style_map) then
      _ReadStyleMap()
    else if (Xml.TagName = ZETag_style_text_properties) then
...
function TZEODSNumberFormatReader.TryGetFormatStrByNum(const DataStyleName: string; out retFormatStr: string): boolean;
var
  i: integer;
begin
  Result := false;
...
function TZEODSNumberFormatWriter.TryGetNumberFormatName(StyleID: integer; out NumberFormatName: string): boolean;
var
  i: integer;
begin
  Result := false;
    ...
    case (_nfType and $FF) of
      ZE_NUMFORMAT_IS_NUMBER:
        begin
          ...
          else if (_nfType and ZE_NUMFORMAT_NUM_IS_PERCENTAGE = ZE_NUMFORMAT_NUM_IS_PERCENTAGE) then
...
begin
  ...
  else if (NumProperties = ZE_NUMFORMAT_NUM_IS_CURRENCY) then
